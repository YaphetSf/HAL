...
/// Candidate-window colors and the mode-feedback artwork: one colour-scheme card whose
...
struct AppearanceSettingsView: View {
    ...
    @State private var choice = SettingsStore.load().modeFeedback
    @State private var artwork = ModeFeedbackArtwork.image(for: SettingsStore.load().modeFeedback)
    @State private var customNames = ModeFeedbackArtwork.installedNames
    @State private var replayID: UInt = 1
    @State private var isChoosingFile = false
    @State private var isTargeted = false
    @State private var importError: String?

    private let previewCandidates = [Candidate(text: "你好", comment: nil),
                                     Candidate(text: "尼", comment: nil),
                                     Candidate(text: "妮", comment: nil)]
    ...
    // MARK: Mode feedback (D26)

    private var modeFeedback: some View {
    ...
    private var stage: some View {
        ZStack {
            ...
            ModeFeedbackScene(
                presentation: ModeFeedbackPresentation(id: replayID, mode: previewMode,
                                                       scheme: scheme),
                artwork: artwork,
                placement: ModeFeedbackPlacement(origin: .zero,
                                                 horizontalDirection: .right,
                                                 verticalDirection: .below),
                style: ModeFeedbackStyle(choice)
            )
            .id(replayID)
        }
        .frame(height: 150)
        ...
        .onTapGesture { replay() }
        // The feedback is a one-shot: without a loop the stage sits empty between taps.
        .onReceive(Timer.publish(every: 2.4, on: .main, in: .common).autoconnect()) { _ in
            replay()
        }
    }

    private var previewMode: ModeFeedbackMode {
        switch replayID % 3 {
    ...
    private func tile(_ tileChoice: ModeFeedbackChoice, title: String,
                      symbol: String = "photo") -> some View {
        ArtworkTile(title: title,
                    image: ModeFeedbackArtwork.image(for: tileChoice),
                    symbol: symbol,
    ...
    private func customTile(_ name: String) -> some View {
        ArtworkTile(title: ModeFeedbackArtwork.index(of: name).map { "Custom \($0)" } ?? "Custom",
                    image: ModeFeedbackArtwork.image(for: .custom(name)),
                    symbol: "photo",
    ...
    private func select(_ newChoice: ModeFeedbackChoice) {
        choice = newChoice
        artwork = ModeFeedbackArtwork.image(for: newChoice)
        SettingsStore.saveModeFeedback(newChoice)
        importError = nil
        replay()
    }
    ...
    private func remove(_ name: String) {
        ...
        customNames = ModeFeedbackArtwork.installedNames
    ...
    private func importArtwork(from url: URL) {
        ...
        do {
            let name = try ModeFeedbackArtwork.install(from: url)
            customNames = ModeFeedbackArtwork.installedNames
            select(.custom(name))
        } catch ModeFeedbackArtwork.ImportError.tooLarge {
...
private struct MiniCandidateBar: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            item("你好", highlighted: true)
            item("尼", highlighted: false)
            item("妮", highlighted: false)
        }
        .padding(5)
